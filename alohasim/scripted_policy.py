...
class PickAndTransferPolicy(BasePolicy):
    ...
    def generate_trajectory(self, ts_first):
        ...
        box_xyz = box_info[:3]
        box_quat = box_info[3:]
        # print(f"Generate trajectory for box_xyz={box_xyz}")
...
class PickPlaceBottlePolicy(BasePolicy):
    ...
    def generate_trajectory(self, ts_first):
        ...
        # bottle pose
        env_info = np.array(ts_first.observation['env_state'])
        bottle_xyz = env_info[:3]
        bottle_quat = env_info[3:7]
        bottle_angle = Quaternion(bottle_quat).degrees
        box_xyz = env_info[7:10]
        # print(f"Generate trajectory for {bottle_xyz=}, {bottle_quat=}")

        # key pose
        gripper_pick_quat = Quaternion(init_mocap_pose_right[3:])
        gripper_pick_quat = gripper_pick_quat * Quaternion(axis=[0.0, 1.0, 0.0], degrees=-90)
        gripper_pick_quat = gripper_pick_quat * Quaternion(axis=[1.0, 0.0, 0.0], degrees=(bottle_angle - 90))
        watch_left_quat = Quaternion(init_mocap_pose_left[3:]) * Quaternion(axis=[0.0, 1.0, 0.0], degrees=30)
        approach_height = 0.16 # height when approaching the bottle
        drop_height = 0.2 # height when dropping the bottle
        place_xyz = np.array([box_xyz[0], box_xyz[1], drop_height]) # place to box center

        # left traj
        self.left_trajectory = [
            {"t": 0, "xyz": init_mocap_pose_left[:3], "quat": init_mocap_pose_left[3:], "gripper": 0}, # sleep
            {"t": 50, "xyz": init_mocap_pose_left[:3] + np.array([-0.2, 0, 0.1]), "quat": watch_left_quat.elements, "gripper": 0}, # move up to avoid collision between arms
            {"t": 350, "xyz": init_mocap_pose_left[:3] + np.array([-0.2, 0, 0.1]), "quat": watch_left_quat.elements, "gripper": 0}, # sleep
        ]

        # right traj
        self.right_trajectory = [
            {"t": 0, "xyz": init_mocap_pose_right[:3], "quat": init_mocap_pose_right[3:], "gripper": 0}, # sleep
            {"t": 120, "xyz": bottle_xyz + np.array([0, 0, approach_height]), "quat": gripper_pick_quat.elements, "gripper": 1}, # approach the bottle from top
            {"t": 170, "xyz": bottle_xyz + np.array([0, 0, -0.002]), "quat": gripper_pick_quat.elements, "gripper": 1}, # go down
            {"t": 190, "xyz": bottle_xyz + np.array([0, 0, -0.002]), "quat": gripper_pick_quat.elements, "gripper": 0}, # close gripper
            {"t": 200, "xyz": bottle_xyz + np.array([0, 0, -0.002]), "quat": gripper_pick_quat.elements, "gripper": 0}, # stay closed for a while
            {"t": 230, "xyz": bottle_xyz + np.array([0, 0, drop_height]), "quat": gripper_pick_quat.elements, "gripper": 0}, # lift the bottle
            {"t": 280, "xyz": place_xyz, "quat": gripper_pick_quat.elements, "gripper": 0}, # move to place position
            {"t": 310, "xyz": place_xyz, "quat": gripper_pick_quat.elements, "gripper": 1}, # open gripper, drop the bottle
            {"t": 350, "xyz": place_xyz, "quat": gripper_pick_quat.elements, "gripper": 1}, # stay
        ]
